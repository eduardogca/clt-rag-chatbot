...
Recurso
Descrição
Toggle de modo agêntico
Permite alternar entre o pipeline agêntico completo (com avaliação de relevância e retry de query) e o pipeline simples (uma única chamada à API, mais rápido).
Toggle de exibição de artigos
Quando ativo, exibe um painel expansível abaixo de cada resposta listando os artigos da CLT efetivamente utilizados, com trecho legível para conferência.
Tooltips em menções de artigos
Sempre que o LLM menciona um artigo no corpo da resposta (ex: "Art. 130"), basta passar o mouse sobre o texto para visualizar o trecho do artigo correspondente.
...
Gratuito, alta qualidade em português
Embeddings
models/text-embedding-004 (Google)
Mesma API, sem custo extra
Vectorstore
ChromaDB (persistido no repositório)
Sem infraestrutura, todos acessam via GitHub
Framework
LangChain + LangGraph
Padrão acadêmico, bem documentado
Interface
Streamlit Community Cloud
Deploy gratuito, URL pública
Versionamento
GitHub
Tracking de tarefas via Issues
Documento
CLT em PDF
Domínio público, bem estruturado
...
Chave de API do Google AI Studio (obter gratuitamente)
...
Retrieval — recupera k=8 chunks do ChromaDB
Avaliação — pontuação combinada: 0.4 × cosine_similarity + 0.6 × llm_judge
Decisão — se score médio < 0.45 e tentativas < 3, reformula a query e repete
Geração — usa apenas chunks com score ≥ 0.45 para gerar a resposta
...
Retrieval — recupera k=6 chunks do ChromaDB
Geração — passa todos os chunks ao LLM e gera a resposta
...
Keyword Accuracy
10/10 (100%)
Resposta contém as palavras-chave esperadas
Recall@6
8/8 (100%)
Artigo esperado está entre os 6 chunks recuperados
Rastreabilidade
10/10
Artigo correto foi utilizado na geração da resposta
...
Módulo
Felipe Teodoro
Data pipeline: extração, chunking e embeddings
src/ingestion/ + notebooks/01
Pessoa 2
RAG core: retrieval, prompts, avaliação e LangGraph
src/retrieval/ + notebooks/02
Pessoa 3
Interface Streamlit, deploy e documentação
src/app/ + README
